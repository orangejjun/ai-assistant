...
| 탭 | 기능 |
|----|------|
| 💬 채팅 | 문서 기반 RAG 질의응답 · **멀티턴 대화 컨텍스트** · Naver 웹 검색 토글 · 임베딩 기반 추천 질문 · 대화 이력 저장/불러오기 |
| 🎨 포스터 생성 | K-beauty 특화 프롬프트 → gpt-image-1 생성 · 크기 3종 선택 · **생성 히스토리 누적** |
| 📊 PPT 생성 | 주제 + 슬라이드 수 입력 → python-pptx .pptx 파일 다운로드 · **생성 히스토리 누적** |
...
| Phase 22 ✅ | 멀티턴 대화 컨텍스트 — 최근 10턴 이력을 GPT에 전달, 연속 대화 가능 |
| (개선) ✅ | 사이드바 UI 전면 개선 — 호버 아이콘, 연한 회색 카드, 자동 새로고침 |
| (개선) ✅ | 포스터/PPT 생성 히스토리 — 세션 내 생성 결과 누적 표시 |
